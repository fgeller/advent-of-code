...
(define input 
  '(
   ...
   (4 10 #\t "tmbpgnmtwksfwcmttbwt")
   (3 5 #\c "mdcncrcj")
   (9 10 #\f "ffffgsfqfmf")
   (8 9 #\t "dzpdqtttqtptltttm")
   (8 9 #\h "wwpqhshhhbv")
   (12 17 #\s "sssssmsztqtbspssds")
   (11 13 #\n "pnndnbnnvwznnnxsnnnn")
   (6 10 #\n "nmxsvjlfnd")
   (2 7 #\h "thlghzhf")
   ...
   (3 14 #\t "jttzstslpnvfftwmqqlq")
   (4 10 #\l "tstfqpvlcl")
   (6 16 #\t "cdtvtttrttwhmtvt")
   (4 7 #\b "lbbwbbm")
   (3 9 #\z "zrqbqzkdz")
...
(println
 (count
  (lambda (line)
    (match line
      [(list min max ch pass)
       (let ((cnt (count (lambda (c) (char=? ch c))
                         (string->list pass)))
             (fst (char=? ch (string-ref pass (sub1 min))))
             (snd (char=? ch (string-ref pass (sub1 max)))))
         (xor fst snd)
         ;; (and (>= cnt min) (<= cnt max))
         )]))
  input))
